...
SoundHandle AudioSystem::PlaySound(const std::string& soundName, bool looping)
{
    ...
    // If no free channel, find the oldest non-looping sound and stop it
    if (channel == -1)
    {
        ...
        for (int i = 0; i < mChannels.size(); i++)
        {
            if (mChannels[i].IsValid())
            {
                ...
                {
                    if (!oldestHandle.IsValid() || mChannels[i] < oldestHandle)
                    {
        ...
        // If found a non-looping sound, stop it
        if (channel != -1)
        {
            Mix_HaltChannel(channel);
            mHandleMap.erase(mChannels[channel]);
            mChannels[channel].Reset();
        }
        else
        {
            // All channels are looping sounds, can't replace
            SDL_Log("[AudioSystem] No free channels available");
            return SoundHandle::Invalid;
        }
    }
    ...
    {
        SDL_Log("[AudioSystem] Failed to play sound: %s", Mix_GetError());
        return SoundHandle::Invalid;
    }

    // Increment handle and store info
    ++mLastHandle;
